...
function attachmentFileName(
    ...
    callback(null, `${req['user']?.userId ?? 'chat'}-${uniqueSuffix}${extname(file.originalname)}`);
}
...
export class ChatController {
    ...
    @Post('request')
    @UseGuards(JwtGuard)
    sendRequest(@Body() body: { receiverEmail: string }, @Req() req) {
        return this.chatService.sendRequest(req.user.userId, body.receiverEmail);
    }

    @Post('accept')
    @UseGuards(JwtGuard)
    accept(@Body() body: { requestId: string }, @Req() req) {
        return this.chatService.acceptRequest(body.requestId, req.user.userId);
    }

    @Post('reject')
    @UseGuards(JwtGuard)
    reject(@Body() body: { requestId: string }, @Req() req) {
        return this.chatService.rejectRequest(body.requestId, req.user.userId);
    }
